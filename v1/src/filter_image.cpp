...
Image make_gaussian_filter(float sigma)  //sigma 1
{
    // TODO: Implement the filter
...
void feature_normalize(Image& im){ //scalare i valori in modo che siano nell'intervallo [0,1]
    assert(im.w*im.h); // assure we have non-empty image

    for (int ch = 0; ch < im.c; ch ++) {

        float vmin = im(0,0,ch);
        float vmax = im(0,0,ch);

        for (int y = 0; y < im.h; y ++) {
            for (int x = 0; x < im.w; x ++) {
                vmin = min (vmin, im(x,y,ch));
                vmax = max (vmax, im(x,y,ch));
            }
        }
        ...
        for (int y = 0; y < im.h; y ++) {
            for (int x = 0; x < im.w; x++) {
                if (range) {
                    float elem = (im(x,y,ch) - vmin) / range;
                    im(x,y,ch) = elem;
                }
                else
                    im(x,y,ch) = 0;
